...
///  Check one term position for substitution compatibility.
///  Returns pair(valid, pair(new_t_enc, new_t_set)).
///  NOTE: Defined BEFORE check_subst_atomic_terms for Z3 axiom ordering.
pub open spec fn check_subst_one_term(
    ...
    CompSpec::IfZero {
        ...
        then_br: Box::new(cs_pair(cs_eq(phi_term, result_term), cs_pair(t_enc, t_set))),
        //  phi_term == var: discover or verify t
        else_br: Box::new(CompSpec::IfZero {
            cond: Box::new(t_set),
            //  First occurrence: discover t
            then_br: Box::new(cs_pair(cs_const(1), cs_pair(result_term, cs_const(1)))),
            //  Subsequent: verify consistency
            else_br: Box::new(cs_pair(cs_eq(result_term, t_enc), cs_pair(t_enc, t_set))),
        }),
    }
}
