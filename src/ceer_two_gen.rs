...
// ============================================================
// Backward direction: equiv in T_G → CEER equiv
// ============================================================
//
// This is the hard direction. It requires showing that
// universal_word(n) ≡ universal_word(m) in T_G implies n ≡ m.
//
// The proof uses:
// 1. Universal words y⁻ⁿ x yⁿ are distinct in the free group F₂
// 2. The only relators are image_relator(a, b) for CEER pairs
// 3. By Britton's Lemma / Magnus subgroup theorem, the
//    interaction between relators preserves the CEER structure
//
// This is axiomatized since it requires Britton's Lemma
// (or normal form arguments for free groups with relators).

/// Backward direction: if universal_word(n) ≡ universal_word(m) in T_G,
/// then n ≡ m in the CEER.
///
/// Justified by Britton's Lemma: the map gen(n) ↦ y⁻ⁿxyⁿ embeds
/// the CEER group faithfully into the 2-generator group T_G.
#[verifier::external_body]
pub proof fn axiom_two_gen_backward(e: CEER, n: nat, m: nat)
    requires
        ceer_wf(e),
        equiv_in_two_gen(e, universal_word(n), universal_word(m)),
    ensures
...
/// Full biconditional: ceer_equiv(n, m) ↔ universal_word(n) ≡ universal_word(m) in T_G.
pub proof fn lemma_two_gen_iff(e: CEER, n: nat, m: nat)
    requires
        ceer_wf(e),
    ensures
        ceer_equiv(e, n, m) <==>
            equiv_in_two_gen(e, universal_word(n), universal_word(m)),
{
    ...
    if equiv_in_two_gen(e, universal_word(n), universal_word(m)) {
        axiom_two_gen_backward(e, n, m);
    }
}
