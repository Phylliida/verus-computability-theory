...
///  Quantifier (tags 7-8), bound var != subst var: push sub-pair.
///  After fix: checker correctly extracts var (4 snds).
pub proof fn lemma_subst_process_pair_quantifier(
    ...
    requires
        ...
        //  Bound var != substitution var → push sub-pair branch
        unpair1(unpair2(phi_node)) != var,
    ensures ({
        let entry = pair(phi_node, result_node);
        let stack = pair(entry + 1, rest);
        let acc = pair(stack, pair(valid, pair(t_enc, t_set)));
    ...
    }),
{
    ...
    //  cs_eq(bound_var, var) = cs_eq(v, var) = 0 because v != var
...
///  Quantifier (tags 7-8), bound var == subst var: exact match.
///  subst returns formula unchanged → phi_node == result_node → check passes.
pub proof fn lemma_subst_process_pair_quantifier_bound(
    i: nat, phi_node: nat, rest: nat,
    valid: nat, t_enc: nat, t_set: nat,
    phi_enc: nat, result_enc: nat, var: nat,
)
    requires
        valid > 0,
        unpair1(phi_node) >= 7,
        //  Bound var == subst var → formula unchanged → phi_node == result_node
        unpair1(unpair2(phi_node)) == var,
    ensures ({
        let entry = pair(phi_node, phi_node);  //  result_node == phi_node (unchanged)
        let stack = pair(entry + 1, rest);
        let acc = pair(stack, pair(valid, pair(t_enc, t_set)));
        let input = pair(i, pair(acc, pair(phi_enc, pair(result_enc, var))));
        eval_comp(check_subst_process_pair(), input)
...
{
    hide(eval_comp);
    let entry = pair(phi_node, phi_node);
    let stack = pair(entry + 1, rest);
    let acc = pair(stack, pair(valid, pair(t_enc, t_set)));
    let orig = pair(phi_enc, pair(result_enc, var));
    let input = pair(i, pair(acc, orig));
    ...
    lemma_subst_dispatch_compound(i, phi_node, phi_node, rest,
        valid, t_enc, t_set, phi_enc, result_enc, var);
    ...
    assert(eval_comp(cs_fst(entry_cs), input) == phi_node) by {
        reveal(eval_comp); lemma_eval_fst(entry_cs, input); lemma_unpair1_pair(phi_node, phi_node);
    }
    assert(eval_comp(cs_snd(entry_cs), input) == phi_node) by {
        reveal(eval_comp); lemma_eval_snd(entry_cs, input); lemma_unpair2_pair(phi_node, phi_node);
    }
    ...
    //  bound_var == var → cs_eq returns 1 → IfZero else branch (exact match)
    let bound_var_cs = cs_fst(phi_content_cs);
    assert(eval_comp(bound_var_cs, input) == var) by {
        reveal(eval_comp); lemma_eval_fst(phi_content_cs, input);
    }
    let var_cs = cs_snd(cs_snd(cs_snd(cs_snd(CompSpec::Id))));
    assert(eval_comp(var_cs, input) == var) by {
    ...
    let bound_eq_var = cs_eq(bound_var_cs, var_cs);
    assert(eval_comp(bound_eq_var, input) == 1) by {
        reveal(eval_comp); lemma_eval_eq(bound_var_cs, var_cs, input);
    }

    //  phi_node == result_node (identical) → cs_eq returns 1
    assert(eval_comp(cs_eq(cs_fst(entry_cs), cs_snd(entry_cs)), input) == 1) by {
        reveal(eval_comp); lemma_eval_eq(cs_fst(entry_cs), cs_snd(entry_cs), input);
    }
    //  cs_and(tags_match, cs_eq(phi_node, result_node)) = 1 * 1 = 1
    assert(eval_comp(cs_and(tags_match, cs_eq(cs_fst(entry_cs), cs_snd(entry_cs))), input) == 1) by {
        reveal(eval_comp);
        lemma_eval_cs_and(tags_match, cs_eq(cs_fst(entry_cs), cs_snd(entry_cs)), input);
    }
    ...
    //  Build compound result: IfZero(is_quant=1) → else → IfZero(bound_eq_var=1) → else → exact match
    let rest_cs = cs_snd(cs_fst(br_acc()));
    assert(eval_comp(check_subst_compound(), input)
        == pair(rest, pair(1nat, pair(t_enc, t_set)))) by {
        reveal(eval_comp);
        //  Build exact match result
        lemma_eval_pair(t_enc_cs, t_set_cs, input);
        lemma_eval_pair(cs_and(tags_match, cs_eq(cs_fst(entry_cs), cs_snd(entry_cs))),
            cs_pair(t_enc_cs, t_set_cs), input);
        ...
        //  Build push-sub-pair then branch (for reference)
        let phi_sub_cs = cs_snd(phi_content_cs);
        let result_content_cs = cs_snd(cs_snd(entry_cs));
        let result_sub_cs = cs_snd(result_content_cs);
        let sub_pair_cs = cs_pair(phi_sub_cs, result_sub_cs);
        ...
        let then_br = cs_pair(
            cs_pair(sp1, rest_cs),
            cs_pair(cs_and(tags_match, cs_eq(bound_var_cs, cs_fst(result_content_cs))),
                cs_pair(t_enc_cs, t_set_cs)));

        //  IfZero(bound_eq_var=1) → else branch (exact match)
