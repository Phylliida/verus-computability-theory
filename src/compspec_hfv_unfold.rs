    ...
    }),
{
    ...
    //  Structural equality (should work now with correct definition order)
    assert(has_free_var_comp() == cs_comp(cs_snd(CompSpec::Id), br));

    lemma_eval_comp(cs_snd(CompSpec::Id), br, input);
    lemma_eval_bounded_rec(f_enc_expr, base_cs, has_free_var_step(), input);
    ...
    let br_result = compspec_iterate(has_free_var_step(), f_enc, base_val, input);
