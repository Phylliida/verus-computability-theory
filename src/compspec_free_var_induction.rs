...
///  unpair2(compspec_iterate(step, f_enc, base_val, input)).
proof fn lemma_hfv_unfold(f_enc: nat, v: nat)
    ensures ({
        let input = pair(f_enc, v);
    ...
    }),
{
    ...
    lemma_eval_bounded_rec(f_enc_expr, base_expr, has_free_var_step(), input);
    ...
    //  Now Z3 knows: eval(BoundedRec, input) = compspec_iterate(step, f_enc, base_val, input)
    //  And: eval(has_free_var_comp(), input) = unpair2(eval(BoundedRec, input))
    lemma_eval_snd(CompSpec::Id,
        compspec_iterate(has_free_var_step(), f_enc,
            pair(pair(f_enc + 1, 0nat), 0nat), input));
}
